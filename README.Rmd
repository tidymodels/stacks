...
To be used in the same ensemble, each of these model definitions must share the same _resample_. This [rsample](https://rsample.tidymodels.org/) `rset` object, when paired with the model definitions, can be used to generate the tuning/fitting results objects for the candidate _ensemble members_ with [tune](http://tune.tidymodels.org/).
...
The API for the package closely mirrors these ideas. See the `basics` vignette for an overview of how this grammar is implemented!
