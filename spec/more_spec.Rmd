...
`get_all_pred()` is an alternative to `get_best_pred()` where, instead of selecting the best sub-model and then calling `collect_predictions()` on it, the function collects the predictions from all of the sub-models (i.e. regardless of choice of hyperparameter). Perhaps, instead of two functions, whether or not to use all predictions would be an argument to `stack_predictions()` which defaults to FALSE.
