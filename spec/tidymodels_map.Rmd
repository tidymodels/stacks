...
dials supplies tools for describing and querying tuning parameters (/hyperparameters.) It provides default "reasonable" ranges for many parameters, which can be set with `update(text_label = dials::some_function())`, where `text_label` is the label set in `tune("text label")`, as well as `grid_*()` functions to assist with grid searches for optimal hyperparameters. `range_(g/s)et()` can get/set the proposed range of hyperparameters. `value_seq(n)` and `value_sample(n)` return a set of values that span the range.

When range of values is dependent on data, can just supply no arguments: `mtry()`, `sample_size()`, `num_terms()`, `num_comp()`--set them, once having seen the data, with `finalize()`.

The principal object class of dials is `param`.
