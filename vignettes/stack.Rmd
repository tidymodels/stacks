...
The `new_stack()` function works sort of like the `ggplot()` constructor from ggplot2—the function creates a basic structure that the object will be built on top of—except you'll pipe the outputs rather than adding them with `+`.

The `members_add()` function will add ensemble members to the stack.

```{r}
penguins_stack <- 
  new_stack() %>%
  members_add(lin_reg_res) %>%
  members_add(spline_res) %>%
  members_add(svm_res)
