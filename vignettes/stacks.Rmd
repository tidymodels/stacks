...
The outputs from each of these ensemble members are highly correlated, so the `evaluate_coefficients` method performs regularization to figure out how we can combine the outputs from the stack members to come up with a final prediction.

```{r, eval = FALSE}
penguins_fit <-
  penguins_stack %>%
  evaluate_coefficients(penguins_train)
