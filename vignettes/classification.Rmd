...
Let's plot the data to get a sense for how separable these three species groups are.
...
ggplot(penguins) +
  ...
  geom_point() +
  labs(x = "Bill Length (mm)", y = "Bill Depth (mm)", col = "Species")
```

Just with these two predictors, it seems like we can already separate these species pretty well! Let's see how well the stacked ensemble can classify these penguins.
