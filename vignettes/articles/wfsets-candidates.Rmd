...
stacks provides a shorthand interface for supplying multiple
sets of candidate members in one call to `add_candidates()` via the 
[workflowsets](https://workflowsets.tidymodels.org/) package.

This example will parallel that shown in  [The "Getting Started" vignette](https://stacks.tidymodels.org/articles/basics.html),
...
Starting with the basic recipe, we convert categorical variables to dummy variables, remove columns with only one observation, impute missing values in numeric variables using the mean, and normalize numeric predictors. Pre-processing instructions for the remaining models are defined similarly.
